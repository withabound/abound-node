import { Mailing } from "../../src/abound";
  ...
  for (const abound of createAboundClients()) {
    ...
    describe("list", () => {
      it("returns a promise that resolves to an object that includes a list of the document's Mailings on success", async () => {
        const mailings: AboundBulkResponse<Mailing> =
          await abound.mailings.list(TEST_USER_ID, TEST_DOCUMENT_ID);
    ...
    describe("retrieve", () => {
      it("returns a promise that resolves to an object that includes a Mailing on success", async () => {
        ...
        expect(normalizeNonIdempotentFields(mailing.data))
          .toMatchInlineSnapshot(`
        Object {
          "createdTimestamp": 1630000000000,
          "from": Object {
