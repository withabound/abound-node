...
```typescript
import { AboundApiClient } from "abound";

const client = new AboundApiClient({ token: "YOUR_TOKEN" });
...
```typescript
import { AboundApi } from "abound";

const request: AboundApi.AccessTokenRequestSchema = {
    ...
};
```
...
```typescript
import { AboundApiError } from "abound";

try {
    await client.form1099Int.create(...);
} catch (err) {
    if (err instanceof AboundApiError) {
...
const client = new AboundApiClient({
    ...
